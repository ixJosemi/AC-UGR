...
int main(int argc, char ** argv){
    ...
    double ncgt; // para el tiempo de ejecución

    // Leer argumento de entrada (nº de componentes del vector)
    if(argc < 2){
        printf("Faltan nº de componentes del vector\n");
        exit(-1);
    }

    unsigned int N = atoi(argv[1]); // Máximo N =2^32-1=4294967295 (sizeof(unsigned int) = 4 B)
    #ifdef VECTOR_LOCAL // Tamaño variable local en tiempo de ejecución ...
    double v1[N], v2[N], v3[N]; // disponible en C a partir de actualización C99
    #endif
    #ifdef VECTOR_GLOBAL
    if(N>MAX)
        N = MAX;
